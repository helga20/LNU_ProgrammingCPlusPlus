...
// випробування ієрархії об'ємних фігур
	...
	std::cout << "Piramid before copying: " << piramid 
		<< "\n Sb = " << piramid.sideSquare() << ";  V = " << piramid.volume() << '\n';
	RectPiramid copyPir(piramid);
	std::cout << "Copy of the piramid: " << copyPir
		<< "\n Sb = " << copyPir.sideSquare() << ";  V = " << copyPir.volume() << '\n';
	std::cin.get();
...
// випробування контейнера об'ємних фігур
	...
	ShArray B(A); B.printOn(std::cout);
	std::cin.get();
	B.clear();
	...
	std::cout << " * By the method doEach(lambda)\n";
	A.doEach([](VolShape* v){ std::cout << *v << "\n     has volume V = " << v->volume() << '\n'; });
	std::cin.get();

	static double Total = 0.0;
	A.doEach([](VolShape* v) { Total += v->volume(); });
	std::cout << "Total volume by lambda is " << Total << '\n';
	std::cin.get();
...
// випробування винятків
	...
	catch(const ShArray::BadIndex& bi)
	{
